...
function formatDate(n, fmt){
  ...
  var h = n.getHours(), h12 = h % 12; if (h12 === 0) h12 = 12;
  var ap = h >= 12 ? "PM" : "AM";
  var Y = n.getFullYear(), M = n.getMonth() + 1, D = n.getDate();
  var m = n.getMinutes(), s = n.getSeconds();
  var dow = days[n.getDay()], mon = months[n.getMonth()];
  var out = fmt;
  out = out.replace("YYYY", Y).replace("YY", String(Y).slice(-2));
  out = out.replace("MMMM", months[n.getMonth()]).replace("MMM", mon).replace("MM", p(M)).replace("M", M);
  out = out.replace("DD", p(D)).replace("D", D);
  out = out.replace("HH", p(h)).replace("H", h);
  out = out.replace("hh", p(h12)).replace("h", h12);
  out = out.replace("mm", p(m)).replace("m", m);
  out = out.replace("ss", p(s)).replace("s", s);
  out = out.replace("A", ap).replace("a", ap.toLowerCase());
  out = out.replace("dddd", dow).replace("ddd", dow.slice(0, 3)).replace("dd", p(n.getDay() + 1)).replace("d", n.getDay() + 1);
  return out;
}
function tick(){
  var n = new Date();
  var fmt = PREFS.dateFormat || "HH:mm";
  document.getElementById("clock").innerHTML = formatDate(n, fmt) + "<small>" + formatDate(n, "ddd D MMM") + "</small>";
}
function applyPrefs(){
  var b = document.body;
  ...
  if (statsEl) statsEl.style.display = PREFS.showSystemStats ? "flex" : "none";
  tick();
}
...
var SETTING_ROWS = [
  ...
  { key: "showSystemStats", label: "System stats", type: "toggle", fmt: function(v){ return v ? "On" : "Off"; } },
  { key: "dateFormat", label: "Date format", type: "choice", opts: ["HH:mm", "h:mm A", "HH:mm:ss", "h:mm:ss A", "MMM D, HH:mm", "MMM D, h:mm A", "YYYY-MM-DD HH:mm", "DD/MM/YYYY HH:mm"],
...
function commitPrefs(onDone){
  svcCall(SVC, SVC_PREFS_SET_M, {
    ...
    showSystemStats: PREFS.showSystemStats, dateFormat: PREFS.dateFormat
...
function resetAll(){
  PREFS = { accent: "steel", tileSize: "standard", labels: true, clock24: false, sort: "mru", pinned: [], hidden: [], showSystemStats: true, dateFormat: "HH:mm" };
...
function loadPrefs(){
  svcCall(SVC, SVC_PREFS_GET_M, {}, function(d){
    if (d && d.prefs) {
      ...
      if (typeof p.showSystemStats === "boolean") PREFS.showSystemStats = p.showSystemStats;
      if (typeof p.dateFormat === "string") PREFS.dateFormat = p.dateFormat;
...
function tickInterval(){
  return /ss|s/.test(PREFS.dateFormat) ? 1000 : 15000;
}
tick(); setInterval(function(){ tick(); setInterval(arguments.callee, tickInterval()); }, tickInterval());
var tries = 0;
