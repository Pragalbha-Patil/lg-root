...
function formatDate(n, fmt){
  ...
  var tokens = {YYYY:Y, YY:String(Y).slice(-2), MMMM:fullMonths[n.getMonth()], MMM:mon,
    MM:p(M), M:M, DD:p(D), D:D, HH:p(h), H:h, hh:p(h12), h:h12,
    mm:p(m), m:m, ss:p(s), s:s, A:ap, a:ap.toLowerCase(),
    dddd:dow, ddd:dow.slice(0, 3), dd:p(n.getDay() + 1), d:n.getDay() + 1};
  // Replace original tokens once; never interpret letters in the resulting names.
  return fmt.replace(/YYYY|MMMM|dddd|MMM|ddd|YY|MM|DD|HH|hh|mm|ss|dd|M|D|H|h|m|s|A|a|d/g, function(token){ return tokens[token]; });
}
function tick(){
  var n = new Date();
